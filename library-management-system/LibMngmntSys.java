...
/**
 * Library Management Class controlling the CRUD Operation of Books in our
 * System
 */
public class LibMngmntSys {
        ...
        if (choice.equalsIgnoreCase("Y")) {
            ...
            while (itr.hasNext()) {
                ...
                if (equalsIgnoreEmpty(author, book.author) && equalsIgnoreEmpty(department, book.department) &&
                    ...
                    System.out.printf(
                            "%-" + books.nameLength + "s %-" + books.authorLength + "s %-" + books.departmentLength +
                                    "s %-" + books.costLength + "d %-" + books.ttlCountLength + "d %-"
                                    + books.avlCountLength + "d %-"
                                    + books.tagsLength + "s\n",
                            book.name, book.author, book.department, book.cost, book.ttlCount, book.avlCount,
                            book.tags);
                }
            }
            ...
            System.out.printf(
                    "%-" + (books.nameLength + 3) + "s%-" + (books.authorLength + 3) + "s%-"
                            + (books.departmentLength + 3) +
                            "s%-" + (books.costLength + 3) + "s%-" + (books.ttlCountLength + 3) + "s%-"
                            + (books.avlCountLength + 3) + "s%-"
    ...
    public static void searchBook() {
        ...
        while (itr.hasNext()) {
            ...
            if (st.name.equalsIgnoreCase(searcher)) {
                System.out.printf("%-16s %-15s %-18s %-12d %-10s\n", st.name, st.author, st.department, st.cost,
                        st.tags);
            }
        }
    ...
    public static boolean compareCost(String[] inputRange, int bookCost) {
        ...
        // case when user enter more thn one cost eg : ["100", "200"]
            ...
            return bookCost >= Integer.parseInt(inputRange[0].trim())
                    && bookCost <= Integer.parseInt(inputRange[1].trim());
    }
